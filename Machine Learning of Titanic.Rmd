...
```{r}
TS_reduced <- glm(Survived ~ Pclass + Sex + Age_rounded + SibSp, family = binomial, data = train)
TS_full <- glm(Survived ~ Pclass + Sex + as.factor(Age_rounded) + SibSp, family = binomial, data = train)
anova(TS_reduced, TS_full)
1-pchisq(25.89, 7)
```
At p-value < 0.05 ([1] 0.0005269761), we reject the null hypothesis ($H_0: E(Y)=\beta_0+\beta_1 X$, i.e., model fits data). There has evidence of a lack of fit of the Age variable, so we want to transform Age.
...
library(class)
train %>% 
  mutate(Sex = as.numeric(Sex)) -> train1
test %>% 
  mutate(Sex = as.numeric(Sex)) -> test1
# Feature Scaling
train_scale <- scale(train1[, c('Pclass', 'Sex', 'Age_rounded', 'SibSp')])
test_scale <- scale(test1[, c('Pclass', 'Sex', 'Age_rounded', 'SibSp')])
...
# Confusiin Matrix
cm <- table(test1$Survived, classifier_knn)
cm

# Model Evaluation - Choosing K
# Calculate out of Sample error
misClassError <- mean(classifier_knn != test1$Survived)
